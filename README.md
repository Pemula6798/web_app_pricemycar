...
PriceMyCar is a Flask web application to estimate used car prices in Indonesia. It uses a machine learning model to estimate the base price and adjusts it based on a 10-factor physical condition checklist.
...
## Project Structure
- `web_app/`: The web application source code (Flask app, Dockerfile, templates, static assets, model files).
- `evaluasi/`: Contains the Jupyter notebook used for model training and evaluation (`evaluasi.ipynb`).

## Local Setup
1. Go to the `web_app` directory:
   ```bash
   cd web_app
   ```
...
## Model Validation
If a user inputs a brand/model that is not in the database, the system will perform a general segment approximation based on year, transmission, fuel type, and mileage. A warning banner will be displayed on the result page to indicate that the prediction is an approximation.
